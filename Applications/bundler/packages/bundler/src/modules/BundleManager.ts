  ...
  async createBundle (): Promise<[UserOperation[], StorageMap]> {
    ...
    for (const entry of entries) {
      ...
      if (newTotalGas.gt(this.maxBundleGas)) {
        debug('skipping totalGas is above max', newTotalGas, this.maxBundleGas)
        break
      }
      ...
      if (paymaster != null) {
        ...
        if (paymasterDeposit[paymaster].lt(validationResult.returnInfo.prefund)) {
          ...
          debug('paymaster has not enough deposit', paymasterDeposit[paymaster].toString(), validationResult.returnInfo.prefund)
          continue
        }
