...
export async function resolveConfiguration (programOpts: any): Promise<{ config: BundlerConfig, provider: BaseProvider, wallet: Signer }> {
  ...
  const config = mergeConfigs(bundlerConfigDefault, localConfig as Partial<BundlerConfig>, commandLineParams)
  ...
  try {
    ...
    throw new Error(`Unable to read --mnemonic ${config.privateKey}: ${e.message as string}`)
  }
